...
class PM2Monitor {
    ...
    try {
      ...
      const metrics = {
        ...
        summary: {
          ...
          online_processes: processes.filter(p => p.pm2_env.status === 'online').length,
          stopped_processes: processes.filter(p => p.pm2_env.status === 'stopped').length,
          errored_processes: processes.filter(p => p.pm2_env.status === 'errored').length,
          total_restarts: processes.reduce((sum, p) => sum + p.pm2_env.restart_time, 0),
          total_memory: processes.reduce((sum, p) => sum + p.monit.memory, 0),
          average_cpu: processes.length > 0 ? 
            processes.reduce((sum, p) => sum + p.monit.cpu, 0) / processes.length : 0
        }
      };
    ...
    try {
      ...
      metrics.processes.forEach(proc => {
        ...
        if (proc.memory > 500 * 1024 * 1024) { // 500MB
          issues.push(`Process ${proc.name} is using ${Math.round(proc.memory / 1024 / 1024)}MB memory`);
        }
  ...
  async autoRestart() {
    try {
      ...
      for (const proc of processes) {
        ...
        if (proc.pm2_env.restart_time > 20 || proc.monit.memory > 800 * 1024 * 1024) {
  ...
  async startMonitoring(interval = 60000) { // Default 1 minute
