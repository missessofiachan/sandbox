...
const PRODUCTS_CACHE_DURATION = Number(process.env.PRODUCTS_CACHE_DURATION) || 300; // 5 minutes
...
router.get('/', cacheResponse(PRODUCTS_CACHE_DURATION), productController.getAllProducts);
// Get a single product by ID (public) - cached for 1 minute
router.get('/:id', cacheResponse(PRODUCT_CACHE_DURATION), productController.getProductById);
