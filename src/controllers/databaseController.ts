    ...
    const poolStats = {
      ...
      mongodb: {
        enabled:
          ...
          process.env.ENABLE_MONGO_MONITORING === 'true',
      ...
      mssql: {
        enabled:
          ...
          process.env.ENABLE_MSSQL_MONITORING === 'true',
        ...
        configuration: {
          ...
          acquireTimeoutMS:
            Number(process.env.MSSQL_ACQUIRE_TIMEOUT_MS) || 60000,
 ...
 * @access Admin only
 */
export const resetConnectionPools = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
function generatePoolRecommendations(health: DatabaseHealth): string[] {
  ...
  if (health.mongodb.connected && health.mongodb.stats) {
    ...
    if (stats.readyState !== 1) {
      recommendations.push(
        'MongoDB connection is not in ready state - consider checking network connectivity'
      );
    }

    if (stats.poolConnections === 'unknown') {
      recommendations.push(
        'Unable to retrieve MongoDB pool statistics - consider updating MongoDB driver'
      );
    }
  }
  ...
  if (health.mssql.connected && health.mssql.stats) {
    ...
    if (poolMax > 20) {
      recommendations.push(
        'MSSQL pool size is quite large - consider reducing if not under heavy load'
      );
    }

    if (idleTimeout < 10000) {
      recommendations.push(
        'MSSQL idle timeout is very low - consider increasing to reduce connection churn'
      );
    }
  }
  ...
  if (nodeEnv === 'production') {
    recommendations.push(
      'Ensure connection pool monitoring is enabled in production'
    );
    recommendations.push(
      'Consider implementing connection pool metrics collection'
    );
  }
